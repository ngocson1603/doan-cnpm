...
namespace GUI.XuLy
{
    public class ExcelExportBH
    {
        ...
        public const string T_BieuMau = "BieuMauBH";
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer)
        {
            if (pReplacer != null)
            {
                ...
                pReplacer.Add("%TenNguoiLap", frmBanSanPham.ten);
                pReplacer.Add("%TenNhanVien", frmBanSanPham.tennv);

                pReplacer.Add("%TongTien", string.Format("{0}", frmBanSanPham.tongtien));
                pReplacer.Add("%MaDonHang", string.Format("{0}", frmBanSanPham.mahd));
                pReplacer.Add("%MaKhachHang", string.Format("{0}", frmBanSanPham.taikhoan));
                pReplacer.Add("%MaNhanVien", string.Format("{0}", frmBanSanPham.manv));
            }
        }
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
            ...
            switch (viewName)
            {
                ...
                case T_BieuMau:
                    ...
                    arrByte = File.ReadAllBytes("BieuMauBH.xlsx").ToArray();
                    break;
        ...
        public bool ExportKhoa(List<View_BieuMauBaoHanh> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            ...
            return OutSimpleReport(dataSource, replacer, "BieuMauBH", isPrintPreview, ref fileName);
        }
