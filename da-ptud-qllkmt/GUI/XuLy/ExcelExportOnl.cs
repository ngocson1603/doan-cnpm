    ...
    public class ExcelExportOnl
    {
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer)
        {
            if (pReplacer != null)
            {
                ...
                pReplacer.Add("%TenNguoiLap", frmThemHoaDonTuDonDatHang.ten);
                pReplacer.Add("%TenNhanVien", frmThemHoaDonTuDonDatHang.tennv);

                pReplacer.Add("%TongTien", string.Format("{0}", frmThemHoaDonTuDonDatHang.tongtien));
                pReplacer.Add("%MaDonHang", string.Format("{0}", frmThemHoaDonTuDonDatHang.mahd));
                pReplacer.Add("%MaKhachHang", string.Format("{0}", frmThemHoaDonTuDonDatHang.taikhoan));
                pReplacer.Add("%MaNhanVien", string.Format("{0}", frmThemHoaDonTuDonDatHang.manv));
            }
        }
        ...
        public bool ExportKhoa(List<View_BieuMauGio> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
